...
function formatShortName(first_name?: string, last_name?: string) {
...
function formatLongName(first_name?: string, last_name?: string) {
  return [first_name, last_name].filter(Boolean).join(' ') || 'Unknown uploader'
}
  ...
  useEffect(() => {
    async function loadData() {
      ...
      const [{ data: media, error: mediaError }, { data: attendees, error: attendeesError }] =
        await Promise.all([
          supabase.from('media').select('*').order('taken_at', { ascending: false }),
          supabase.from('attendees').select('attendee_id, first_name, last_name, profile_photo_url'),
        ])
      ...
      const attendeeMap = new Map<string, AttendeeRow>()
      ;(attendees || []).forEach((person: AttendeeRow) => {
        attendeeMap.set(person.attendee_id, person)
      })

      const merged: GalleryItem[] = (media || []).map((item: MediaRow) => {
      ...
      merged.forEach((item) => {
        initialDrafts[item.id] = item.location_name || ''
      })

      setDraftLocations(initialDrafts)
      setItems(merged)
      setLoading(false)
    }
    ...
    <div className="gallery-shell">
      <div className="gallery-hero">
        <div className="hero-copy">
          ...
          <p>
            A living gallery of moments, places, and witness. Browse what everyone shared, see
            who uploaded each photo, and revisit the journey with context instead of chaos.
          </p>
        </div>
      ...
      {loading && <p className="status">Loading gallery…</p>}
